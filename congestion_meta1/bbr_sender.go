...
func (b *bbrSender) OnCongestionEvent(number congestion.PacketNumber, lostBytes congestion.ByteCount, priorInFlight congestion.ByteCount) {
	...
	if b.mode == PROBE_BW {
		b.UpdateGainCyclePhase(monotime.Now(), priorInFlight, true)
	}
...
//func (b *bbrSender) OnCongestionEvent(priorInFlight congestion.ByteCount, eventTime monotime.Time, ackedPackets, lostPackets []*congestion.Packet) {
//	totalBytesAckedBefore := b.sampler.totalBytesAcked
//	isRoundStart, minRttExpired := false, false
//
//	if lostPackets != nil {
...
func (b *bbrSender) UpdateBandwidthAndMinRtt(now monotime.Time, number congestion.PacketNumber, ackedBytes congestion.ByteCount) bool {
